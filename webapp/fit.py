"""Per-user résumé-fit scoring (Stage 2b).

Scores a user's résumé against the current job corpus in the database and writes
the result into ``user_job_fit``, so each account sees its own matches. Invoked
three ways:
...
The corpus is the DB's active jobs. That's a subset of the pipeline's full
fetched corpus (which the owner's ingest still scores at higher fidelity into
the ``local`` rows), but it grows as jobs accumulate and needs no re-fetch, so
it's the right trade-off for on-demand and per-user re-scoring.
"""
...
def _load_corpus(conn) -> list[tuple[int, JobPosting]]:
    """Active jobs as (db_id, JobPosting) pairs for scoring. The synthetic key
    (source:company:db_id) is unique per row, which is all score_jobs needs."""
    rows = conn.execute(
        "SELECT id, source, company, title, location, description, posted_at "
        "FROM jobs WHERE is_active = 1").fetchall()
    out = []
    for r in rows:
...
def rescore_user(conn, user_id: str, resume_text: str) -> int:
    """Score ``resume_text`` against the active job corpus and write the result
    into user_job_fit for ``user_id``. Returns the number of jobs scored. A
    ...
    for db_id, jp in corpus:
        db.upsert_user_fit(conn, user_id, db_id, jp.fit_score, jp.rank_score,
                           jp.cluster, now)
    conn.commit()
    return len(corpus)
...
def rescore_all_active_users(conn) -> dict[str, int]:
    ...
    run after ingesting fresh postings. Returns {user_id: jobs_scored}."""
    out: dict[str, int] = {}
    for uid in db.users_with_resume(conn):
        out[uid] = rescore_user(conn, uid, db.get_user_resume(conn, uid))
    return out
